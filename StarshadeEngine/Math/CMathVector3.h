...
namespace Math {
	struct Vector3 {
		...
		// Unary operators.
		inline float& operator [] (size_t index) { v[index]; } 

		inline Vector3 operator + () const { return Vector3(x, y, z); }
		inline Vector3 operator - () const { return Vector3(-x, -y, -z); }

		inline operator const CSIMDVector () const { return CSIMDVector(x, y, z, 0.0f); }
		...
		static Vector3 MoveTowards(const Vector3& from, const Vector3& to, float t) { return from.Lerp(to, t); }
